# **************************************************************************** #
#                                                                              #
#                                                         :::      ::::::::    #
#    script_ex03.sh                                       :+:      :+:    :+:  #
#                                                     +:+ +:+         +:+      #
#    By: idcornua <marvin@42.fr>                    +#+  +:+       +#+         #
#                                                 +#+#+#+#+#+   +#+            #
#    Created: 2018/07/26 19:55:07 by idcornua          #+#    #+#              #
#    Updated: 2019/01/15 16:30:33 by idcornua         ###   ########.fr        #
#                                                                              #
# **************************************************************************** #

#!/bin/bash

# DEFINE VARIABLES
RED='\033[0;31m'
GREEN='\033[0;32m'
YELL='\033[1;33m'
ORNG='\033[0;33m'
BLUE='\033[1;34m'
NC='\033[0m'

J_NUM="02"
EX_NUM="03"
EX_NAME="ft_is_negative"
U_EX="user_${EX_NUM}_exe"
M_EX="moulinette_${EX_NUM}_exe"
FLAGS='-Wall -Wextra -Werror'
J_PATH=`pwd`
SCRIPT_LOC="$( cd -- "$(dirname "$0")" >/dev/null 2>&1 ; pwd -P )"

echo -e "${ORNG}===================== EX03 =====================\n"
echo -e "" > error_log
echo -e "${BLUE}-- Compilation --${NC}\n"
echo -e "$> gcc -o user_exe ${FLAGS} main_ex${EX_NUM}.c ${EX_NAME}.c"

# COMPILE MOULINETTE FILES
gcc -o ${SCRIPT_LOC}/../${M_EX} ${FLAGS} ${SCRIPT_LOC}/../mains/m_ex${EX_NUM}.c ${SCRIPT_LOC}/../references_files/ex${EX_NUM}.c ${SCRIPT_LOC}/../../common/*.c
# COMPILE USER FILES
gcc -o ${U_EX} ${FLAGS} ${SCRIPT_LOC}/../mains/m_ex${EX_NUM}.c ${J_PATH}/ex${EX_NUM}/${EX_NAME}.c ${SCRIPT_LOC}/../../common/*.c  2> error_log

# CHECKING COMPILATION ERRORS
ERROR=`cat error_log`

# STOP HERE IF ERRORS 
if [ "${ERROR}" != "" ]
then
	echo -e "\n====> ${RED}FAILURE${NC} <===="
	echo -e "Does not compile.\n"
	exit
# ELSE PROCEED
else
	echo -e "\n====> ${GREEN}SUCCESS${NC} <===="
	echo -e "Compilation OK.\n"
fi

ARRAY_OF_TEST_VALUES=(-10 -3 -1 0 -1 3 10 -2147483648 2147483647)

for i in "${!ARRAY_OF_TEST_VALUES[@]}";
do 
  echo -e "\n${BLUE}-- Test 0$i --${NC}\n"
  echo -e "$> ./${M_EX} ${ARRAY_OF_TEST_VALUES[$i]} > m_output"
  echo -e "$> ./${U_EX} ${ARRAY_OF_TEST_VALUES[$i]} > u_output"
  echo -e "\n$> diff -U 10 u_output m_output > m_diff\n"

  # EXECUTING BOTH PROGRAMS
  ${SCRIPT_LOC}/../${M_EX} ${ARRAY_OF_TEST_VALUES[$i]} > m_output
  ./${U_EX} ${ARRAY_OF_TEST_VALUES[$i]} > u_output

  # DIFFING THEM OUT
  diff -U 10 u_output m_output > m_${i}_diff
  echo m_${i}_diff >> m_diff

  M_EX_DIFF=`cat m_${i}_diff`
  if [ "${M_EX_DIFF}" != "" ]
  then
    echo -e "${M_EX_DIFF}"
  fi
done

# CHECKING DIFF RESULT
M_DIFF=`cat m_diff`
rm *_output *_diff  *_log *_exe
if [ "${M_DIFF}" != "" ]
then
  echo -e "\n====> ${RED}FAILURE${NC} <===="
  echo -e "Diff KO :(\n"
  exit 0
else
	echo -e "\n====> ${GREEN}SUCCESS${NC} <===="
	echo -e "Diff OK :D\n"
  exit 1
fi